...
#node = sx126x.sx126x(serial_num = "/dev/ttyS0",freq=868,addr=100,power=22,rssi=True,air_speed=2400,relay=False)
node = sx126x.sx126x(serial_num = "/dev/ttyAMA0", freq=frequency, addr=address,power=22,rssi=False,air_speed=2400,relay=False)
...
def send_cpu_continue(continue_or_not = True):
    if (continue_or_not and (not stop)):
        global timer_task
        global seconds
        #
        # broadcast the cpu temperature at 868.125MHz
        ### NFB : Why 12 on the own frequency? Should be 18
        data = bytes([255]) + bytes([255]) + bytes([18]) + bytes([255]) + bytes([255]) + bytes([18]) + "CPU Temperature:".encode()+str(get_cpu_temp()).encode()+" C".encode()
        node.send(data)
        time.sleep(0.2)
...
try:
    time.sleep(1)
    #frequency = 868
    print("This is node [{0},{1}] and will act as a sender".format(address,frequency))
    print("Sending CPU temperature every few seconds to node [{0},{1}]".format(destination,frequency))
