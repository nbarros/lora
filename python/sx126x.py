...
class sx126x:
    ...
    M0 = 22
    M1 = 27
    # if the header is 0xC0, then the LoRa register settings dont lost when it poweroff, 
    # and 0xC2 will be lost. 
    ...
    # Note comm_freq = start_freq + offset_freq
    ...
    def __init__(self,serial_num,freq,addr,power,rssi,air_speed=2400,\
        ...
        msg = """
Initialize serial object with properties:
...
""".format(serial_num,freq,addr,power,rssi,air_speed,net_id,buffer_size,crypt,relay,lbt,wor)
        ...
        # This defines configuration mode 
        # Ref: https://www.waveshare.com/wiki/SX1262_868M_LoRa_HAT#Features
    ...
    def set(self,freq,addr,power,rssi,air_speed=2400,\
        ...
        # Set module into configuration mode
        GPIO.output(self.M0,GPIO.LOW)
        GPIO.output(self.M1,GPIO.HIGH)
        time.sleep(0.1)
...
# "20,868,Hello World"
    def send(self,data):
        ...
        print("Sending communcation [{0}]".format(data))
        self.ser.write(data)
        # if self.rssi == True:
            # self.get_channel_rssi()
        time.sleep(0.1)

    def send_v2(self,address,data):
        # add the node address ,and the node of address is 65535 can konw who send messages
        l_addr = address & 0xff
        h_addr = address >> 8 & 0xff

        msg = bytes([h_addr,l_addr])+data.encode()
        print("Sending communcation [{0}]".format(msg))
    ...
    def receive(self):
        if self.ser.inWaiting() > 0:
            time.sleep(0.5)
            r_buff = self.ser.read(self.ser.inWaiting())
            print("Raw message [{0}]".format(r_buff))
